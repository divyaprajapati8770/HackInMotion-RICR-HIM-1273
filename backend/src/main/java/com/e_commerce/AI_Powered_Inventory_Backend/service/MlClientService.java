...
import com.e_commerce.AI_Powered_Inventory_Backend.dto.request.ForecastRequest;
...
public class MlClientService {
    ...
    public Optional<MlForecastResponse> requestForecast(List<SalesRecord> history, int horizonDays,
        ...
        try {
            ...
            List<ForecastRequest.SalesPoint> points = history.stream()
                    .map(r -> new ForecastRequest.SalesPoint(r.getSaleDate().format(fmt), r.getUnitsSold()))
                    .toList();

            ForecastRequest req = new ForecastRequest(points, horizonDays, leadTimeDays,
