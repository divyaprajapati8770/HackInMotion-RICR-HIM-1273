...
/** Envelope with an outbound paper plane + pulsing dots — "check your inbox" pending state, distinct from the checkmark-envelope success state below. */
export function CheckInboxIllustration({ className }: { className?: string }) {
  return (
    <IllustrationFrame glow="indigo" className={className}>
      <svg viewBox="0 0 240 180" fill="none" xmlns="http://www.w3.org/2000/svg" className="w-full">
        <ellipse cx="120" cy="150" rx="70" ry="9" className="fill-slate-900/5 dark:fill-black/30" />
        <rect x="50" y="60" width="140" height="86" rx="10" className="fill-indigo-50 dark:fill-white/[0.04]" stroke="currentColor" strokeOpacity="0.1" />
        <path d="M50 68L120 112L190 68" stroke="currentColor" strokeOpacity="0.25" strokeWidth="3" fill="none" strokeLinecap="round" strokeLinejoin="round" />
        <motion.g
          animate={{ x: [0, 6, 0], y: [0, -4, 0] }}
          transition={{ duration: 2.2, repeat: Infinity, ease: "easeInOut" }}
        >
          <circle cx="176" cy="52" r="28" className="fill-secondary" />
          <path d="M164 52L188 44L180 60L164 52Z" fill="white" />
        </motion.g>
        <motion.circle
          cx="205" cy="30" r="3" className="fill-secondary/60"
          animate={{ opacity: [0.2, 1, 0.2] }}
          transition={{ duration: 1.4, repeat: Infinity, delay: 0 }}
        />
        <motion.circle
          cx="216" cy="40" r="2.5" className="fill-secondary/40"
          animate={{ opacity: [0.2, 1, 0.2] }}
...
/** Envelope with a rising checkmark — email verification success. */
export function EmailVerifiedIllustration({ className }: { className?: string }) {
  return (
    <IllustrationFrame glow="emerald" className={className}>
      <svg viewBox="0 0 240 180" fill="none" xmlns="http://www.w3.org/2000/svg" className="w-full">
        <ellipse cx="120" cy="150" rx="70" ry="9" className="fill-slate-900/5 dark:fill-black/30" />
        <rect x="50" y="60" width="140" height="86" rx="10" className="fill-emerald-50 dark:fill-white/[0.04]" stroke="currentColor" strokeOpacity="0.1" />
        <path d="M50 68L120 112L190 68" stroke="currentColor" strokeOpacity="0.25" strokeWidth="3" fill="none" strokeLinecap="round" strokeLinejoin="round" />
        <circle cx="176" cy="52" r="28" className="fill-tertiary" />
        <path d="M162 52L172 62L192 40" stroke="white" strokeWidth="5" strokeLinecap="round" strokeLinejoin="round" />
      </svg>
    </IllustrationFrame>
  );
}
